...
{
    public:
        DcMotor(int pin_fwd, int pin_bkwd, Direction dir=FORWARD, int pwm_resolution=8);
        void init();
    ...
    private:
    	int pwm_res;  //number of bits in PWM resolution
    	int max_pwm_value;
        float power;
        int direction;
...
class EncoderDcMotor
{
    public:
        ...
        Odometer odometer;
        PID pid_controller;
        EncoderDcMotor(int pin_fwd, int pin_bkwd, Direction dir, int pwm_resolution=8,\
